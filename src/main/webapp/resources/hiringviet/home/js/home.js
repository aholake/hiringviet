...
function showResultJobHot(response) {
	...
	if (FAIL == response.result) {
		...
		for (var index = 0; index < jobListResponse.length; index++) {
			...
			var item = '<div class="">\
							<div class="job-box">\
								<div class="location-sticky orange darken-1">'
					+ jobListResponse[index].company.address.ward.district.province.provinceName
					+ '</div>\
									<div class="row none-margin-bottom">\
									<div class="col m3 center hide-on-med-and-down">\
									<a href="/company/'
					+ jobListResponse[index].company.id
					+ '"><img src="'
					+ jobListResponse[index].company.avatar
					+ '" class="responsive-img company-logo"></a>\
					...
					+ jobListResponse[index].id
					+ '">'
					+ jobListResponse[index].title
					+ '</a>\
											</h1>\
										</div>\
										<a href="#" class="company-name">'
					+ jobListResponse[index].company.displayName
					+ '</a>\
										<p class="work-location"><a href="#">'
					+ jobListResponse[index].company.address.ward.district.province.provinceName
					+ '</a></p>\
										<div class="job-info">\
											<div class="row">\
												<div class="col m6 none-padding-left">\
													<p><i class="material-icons prefix-icon">attach_money</i>'
					+ $('#text_title_salary').val()
					+ ': \
														<span class="info">'
					+ jobListResponse[index].minSalary
					+ ' - '
					+ jobListResponse[index].maxSalary
					+ '</span>\
													</p>\
												</div>\
												<div class="col m6 none-padding-left">\
													<p class="right"><i class="material-icons prefix-icon">date_range</i>'
					+ $('#text_title_post_date').val()
					+ ': \
														<span class="info">'
					+ jobListResponse[index].postDate
					+ '</span>\
													</p>\
												</div>\
												<div class="col m6 none-padding-left">\
													<p><i class="material-icons prefix-icon">loyalty</i>'
					+ $('#text_title_major').val()
					+ ': \
														<span class="info">'
					+ jobListResponse[index].position.displayName
					+ '</span>\
													</p>\
												</div>\
												<div class="col m6 none-padding-left">\
													<p class="right"><i class="material-icons prefix-icon">people</i>'
					+ $('#text_total_employee').val()
					+ ': \
														<span class="info">'
					+ jobListResponse[index].size
					+ $('#text_title_people').val()
					+ '</span>\
													</p>\
												</div>\
											</div>\
